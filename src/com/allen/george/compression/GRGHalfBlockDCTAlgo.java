...
public class GRGHalfBlockDCTAlgo extends GRGCompressionAlgorithm {

	// Constructor
	// Takes in an input matrix (the original)
	public GRGHalfBlockDCTAlgo(Mat original) {
		...
		final Mat borderdImage = Mat.zeros(sizeWithBorders, CvType.CV_32FC1);// create
																				// a
																				// matrix
																				// with
																				// the
		...
		for (int y = 0; y < original.rows(); y++) { // loop through the rows of
			...
			for (int x = 0; x < original.cols(); x++) { // loop through the cols
														// of the image
				borderdImage.put(y, x, new double[] { original.get(y, x)[0] }); // put
																				// the
																				// pixel
																				// data
																				// into
	...
	public void compress() {
		...
		Mat m = Utils.reconstruct(blocks, original); // reconstruct a matrix out
														// of the blocks
		encode(m); // encode the reconstructed matrix
	}
	...
	private List<Mat> preprocess() {
		...
		List<Mat> res = new ArrayList<Mat>(); // init a list of matrices to
												// store the blocks

		for (int y = 0; y < original.rows(); y += 8) { // loop through the
														// original rows 8
														// points at a time
			for (int x = 0; x < original.cols(); x += 8) { // loop through the
															// original cols 8
															// points at a time
				Rect rect = new Rect(x, y, 8, 8); // create a new rect that has
													...
													// and height of 8
				res.add(new Mat(original, rect)); // add the rect to a MAT then
	...
	//Method to encode a matrix using huffman coding
	//Takes in the matix
	//encodes the matrix
	...
	private void encode(Mat m) {
		...
		roots = new Node[1];//init the node array to store the huffman roots
